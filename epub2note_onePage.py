...
class Epub2note:
    ...
    def _gen_book_content(self):
        ...
        for item in self._toc_list:
            ...
            md_item = self._gen_by_href(path=path, title=title, href=item['href'])
            # 开头的**加粗改为##二级标题
            if md_item[:2] == '**':
                md_char = list(md_item)
                md_char = ['#', '#', ' '] + md_char
                md_item = ''.join(md_char)
            # \n\n>引用改为三级标题
            # new_item = re.sub(r'\n\n> ', '\n\n### ', md_item)
            # md_items.append(new_item)
            md_items.append(md_item)
            prev_lvl = item['lvl']
            prev_title = title
        md_doc = '\n\n\n'.join(md_items)
        self._sy_client.create_note(notebook=self._notebook_id, path='/' + self._book_name, markdown=md_doc)
        time.sleep(4)
